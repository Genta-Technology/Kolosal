...
{
    // ImFont::CalcWordWrapPositionA expects unscaled wrap_width.
    return font->CalcWordWrapPositionA(scale, text, text_end, wrap_width);
}

// [UPDATED] Use ImFontBaked for glyph advance, as per current ImGui API.
static ImVec2 FindCharPosition(const char* text_begin, const char* char_position, const char* text_end, const float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
    const float line_height = g.FontSize;
    const float scale = line_height / font->DefaultSize;
    ...
    while (s < text_end)
    {
        ...
        // [UPDATED] Use ImFontBaked for glyph advance
        ImFontBaked* baked = font ? font->GetFontBaked(font->DefaultSize) : nullptr;
        float char_width = 0.0f;
        if (baked && baked->IndexAdvanceX.Data != nullptr)
            char_width = baked->GetCharAdvance((ImWchar)c);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            if (is_multiline)
                text_size = InputTextCalcTextSize(GImGui, text_begin, text_end, NULL, NULL, false, word_wrap_width);
        }
